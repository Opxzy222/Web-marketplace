...
const UpdateShopProducts = () => {
  ...
  const getImageKey = (type, identifier) => `${type}_${identifier}`;
  ...
  const handleRemoveCustomProduct = customId => {
    setCustomProducts(customProducts.filter(p => p.customId !== customId));
    const imgKey = getImageKey('custom', customId);
    setPendingImages(prev => {
      const { [imgKey]: _, ...rest } = prev;
      return rest;
    });
  };
    ...
    setSelectedProducts(prevState => ({
      ...
      [category]: {
        ...
        [subcategory]: selectedOptions.map(option => ({
          ...
          price: prevState[category]?.[subcategory]?.find(p => p.value === option.value)?.price ?? '',
          image: prevState[category]?.[subcategory]?.find(p => p.value === option.value)?.image ?? null,
  ...
  const handleCustomProductCheckboxChange = (customId) => {
    setCustomProducts(prevState => prevState.map(product =>
  ...
  const handleCustomProductPriceChange = (customId, value) => {
    setCustomProducts(prevState => prevState.map(product =>
      product.customId === customId
        ? { ...product, price: value }
        : product
  ...
  const handleCustomProductNameChange = (customId, value) => {
    setCustomProducts(prevState => prevState.map(product =>
      product.customId === customId
  ...
  const handleSubmitProducts = async () => {
    try {
      ...
      customProducts.forEach((customProduct, index) => {
        ...
        const imgKey = getImageKey('custom', customProduct.customId);
        if (pendingImages[imgKey]) {
  ...
  const categorizedCustoms = useMemo(() => {
    const map = {};
    customProducts.forEach(product => {
      if (product.category_name && product.subcategory_name) {
        if (!map[product.category_name]) {
          map[product.category_name] = {};
        }
        if (!map[product.category_name][product.subcategory_name]) {
          map[product.category_name][product.subcategory_name] = [];
        }
        map[product.category_name][product.subcategory_name].push(product);
      }
    });
    return map;
  }, [customProducts]);
  ...
  const uncategorizedCustoms = useMemo(() => {
    return customProducts.filter(product => !product.category_name || !product.subcategory_name);
  }, [customProducts]);
          ...
          <div className="shop-manager-content">
                ...
                <div className="table-wrapper">
                  ...
                  <div className="table-scroll-container">
                    <table className="product-table">
                      ...
                      <tbody>
                        {uncategorizedCustoms.map((product) => (
                          <tr key={product.customId}>
                            <td>
                              {editingCustomId === product.customId ? (
                                <input
                                  className="custom-name-input"
                                  value={product.name}
                                  onChange={(e) => handleCustomProductNameChange(product.customId, e.target.value)}
                                  onBlur={() => setEditingCustomId(null)}
                                  onKeyDown={(e) => e.key === 'Enter' && setEditingCustomId(null)}
                                  autoFocus
                                ...
                                <button 
                                  ...
                                  onClick={() => setEditingCustomId(product.customId)}
                                >
                                  {product.name}
                                </button>
                              )}
                            ...
                            <td>
                              <label className="checkbox-container">
                                <input
                                  ...
                                  onChange={() => handleCustomProductCheckboxChange(product.customId)}
                            ...
                            <td>
                              <button
                                ...
                                onClick={() => {
                                  setModalTargetInfo({ type: "custom", customId: product.customId });
                            ...
                            <td>
                              <ProductImageManager
                                productKey={getImageKey('custom', product.customId)}
                            ...
                            <td>
                              <button
                                onClick={() => {
                                  setPendingDeleteInfo({ type: "custom", customId: product.customId });
              ...
              <div className="custom-product-inputs">
                ...
                <div className="custom-card">
                  ...
                  {isCategoryDropdownOpen && (
                    <div className="updp-modal-overlay" onClick={() => setIsCategoryDropdownOpen(false)}>
                      <div className="updp-modal-content" onClick={(e) => e.stopPropagation()}>
                        <h3 className="updp-modal-title">Select Category</h3>
                        <input
                        ...
                        <button 
                          className="updp-modal-cancel-button"
                          onClick={() => {
                            setIsCategoryDropdownOpen(false);
                            setCategorySearchQuery("");
                          }}
                  ...
                  {isSubcategoryDropdownOpen && (
                    <div className="updp-modal-overlay" onClick={() => setIsSubcategoryDropdownOpen(false)}>
                      <div className="updp-modal-content" onClick={(e) => e.stopPropagation()}>
                        <h3 className="updp-modal-title">Select Subcategory</h3>
                        <input
                          className="search-input"
                  ...
                  {expandedCategories[category] && (
                    <div>
                      {sortedSubcategories[category]?.map((subcategory) => {
                        ...
                        const selected = selectedProducts[category]?.[subcategory] || [];
                        const customsHere = categorizedCustoms[category]?.[subcategory] || [];
                        ...
                        return (
                          <div key={subcategory} className="subcategory-container">
                            ...
                            {dropdownData.isOpen && (
                              <div className="updp-modal-overlay" onClick={() => {
                                setDropdownState((prev) => ({
                                  ...prev,
                                  [dropdownKey]: {
                                    ...dropdownData,
                                ...
                                <div className="updp-modal-content product-modal" onClick={(e) => e.stopPropagation()}>
                                  <input
                                    className="search-input"
                            ...
                            {(selected.length > 0 || customsHere.length > 0) && (
                              <div className="table-wrapper">
                                <table className="product-table">
                                  <thead>
                                    <tr>
                                  ...
                                  <tbody>
                                      ...
                                      <tr key={`sug_${product.value}`}>
                                        ...
                                        <td>
                                          <button
                                            className="price-button"
                                            onClick={() => {
                                              setModalTargetInfo({ type: "suggested", category, subcategory, productId: product.value });
                                              setModalValue(product.price || "");
                                              setShowModal(true);
                                            }}
                                          >
                                        ...
                                        <td>
                                          <button
                                            className="remove-button"
                                            onClick={() => {
                                              setPendingDeleteInfo({ type: "suggested", category, subcategory, productId: product.value });
                                              setShowDeleteModal(true);
                                            }}
                                          >
                                            X
                                    ...
                                    {customsHere.map((product) => (
                                      <tr key={`cus_${product.customId}`}>
                                        <td>
                                          {editingCustomId === product.customId ? (
                                            <input
                                              className="custom-name-input"
            ...
            {showModal && (
              <div className="updp-modal-overlay" onClick={() => {
                ...
                <div className="updp-modal-content" onClick={(e) => e.stopPropagation()}>
                  <h3 className="updp-modal-title">Edit Listing Price</h3>
                  <p className="updp-modal-text">Enter a new price:</p>
                  <input
                    ...
                    autoFocus
                  />
                  <div className="updp-modal-button-row">
                    <button
                      className="updp-modal-confirm-button"
                      onClick={() => {
                        if (modalTargetInfo) {
                          if (modalTargetInfo.type === "custom") {
                            handleCustomProductPriceChange(modalTargetInfo.customId, modalValue);
                          } else if (modalTargetInfo.type === "suggested") {
            ...
            {showDeleteModal && (
              <div className="updp-modal-overlay" onClick={() => {
                ...
                <div className="updp-modal-content" onClick={(e) => e.stopPropagation()}>
                  <h3 className="updp-modal-title">Confirm Delete</h3>
                  <p className="updp-modal-text">Are you sure you want to remove this listing?</p>
                  <div className="updp-modal-button-row">
                    <button
                      className="updp-modal-confirm-button"
                      onClick={() => {
                        if (pendingDeleteInfo) {
                          if (pendingDeleteInfo.type === "suggested") {
                            const { category, subcategory, productId } = pendingDeleteInfo;
                            handleRemoveProduct(category, subcategory, productId);
                            const imgKey = getImageKey('suggestion', productId);
                            setPendingImages((prev) => {
                              const { [imgKey]: _, ...rest } = prev;
                              return rest;
                            });
                          } else if (pendingDeleteInfo.type === "custom") {
                            const { customId } = pendingDeleteInfo;
                            handleRemoveCustomProduct(customId);
                          }
                        }
                        setShowDeleteModal(false);
                        setPendingDeleteInfo(null);
